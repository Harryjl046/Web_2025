...
def main():
    ...
    print("[2] 词项数量对检索的影响:")
    print("  - 查询词项越多 → 查询向量维度越高 → 可能提高精确度但降低召回率")
    print("  - 查询词项过少 → 相似度区分度可能不足 → 需结合IDF权重筛选关键词")
    print()
    
    print("[3] TF-IDF权重的作用:")
    print("  - TF: 反映词项在文档中的重要性(高频词获得更高权重)")
    print("  - IDF: 降低常见词权重,提升区分性词项的重要性")
    print("  - 对比: 若仅用TF,常见词(如'group''meetup')会主导相似度计算")
    print("         使用TF-IDF后,罕见但相关的词项(如'python''hiking')权重提升")
    print()
    
    print("[4] 与布尔检索的对比:")
    print("  布尔检索:")
    print("    优势: 精确匹配,返回结果明确(包含/不包含)")
    print("    劣势: 无相关性排序,结果可能过多或过少,无法处理部分匹配")
    print()
    print("  向量空间模型:")
    print("    优势: 基于相似度排序,支持部分匹配,容错性强")
    print("    劣势: 计算复杂度高(需计算所有文档相似度),相似度阈值难以设定")
    print()
    
    print("[5] 最优查询策略建议:")
    print("  - 词项选择: 优先选择IDF值较高的区分性词项(避免'group''event'等超高频词)")
    print("  - 词项数量: 建议3-5个词项(平衡精确度与召回率)")
    print("  - 混合检索: 先用布尔检索缩小候选集,再用向量空间模型排序")
    print("  - 相似度阈值: 根据实际需求设定(如Top-20或相似度>0.3)")
    print()
